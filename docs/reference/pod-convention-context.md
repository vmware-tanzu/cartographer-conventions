...
The pod convention context is the body of the webhook request and response. The spec is provided by Cartographer Conventions and the status is set by the convention server.
...
Let's expand more on the image config present in `PodConventionContext`. Cartographer Conventions passes along this information for each image in good faith, the controller is not the source of the metadata and there is no guarantee that the information is correct.
