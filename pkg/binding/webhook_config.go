...
func NewClientManager(wc WebhookConfig, grp schema.GroupVersion, addToSchemaFunc func(s *runtime.Scheme) error) (cm webhook.ClientManager, err error) {
	cm, err = webhookutil.NewClientManager([]schema.GroupVersion{grp}, addToSchemaFunc)
	if err != nil {
		return
	}
	cm.SetAuthenticationInfoResolver(wc.AuthInfoResolver)
	cm.SetServiceResolver(wc.ServiceResolver)
	return
}
